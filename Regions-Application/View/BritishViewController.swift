...
//  Regions-Application
//
//  Created by Anton Smirnov on 14.01.2022.
//

import UIKit

class BritishViewController: UIViewController, UIGestureRecognizerDelegate, UITextFieldDelegate {
    ...
    override func viewDidLoad() {
        ...
        textField.leftView = UIImageView(image: UIImage(named: "British_Flag"))
    ...
    private func personData() {
        apiService.responseRegion(endpoints: Constants.Endpoints.British,region: textField.text!) { [self](isSucess, str) in
            if isSucess {
                messages.showMessage(label: label, message: str)
                textField.text?.removeAll()
            } else if (str.contains("Could not connect to the server.")) {
                jsonLoad.loadSpeciesInfoJSOn(resource: "britishPerson", label: label, text: textField.text!)
                textField.text?.removeAll()
            }else {
                messages.showMessage(label: label, message: str)
                textField.text?.removeAll()
