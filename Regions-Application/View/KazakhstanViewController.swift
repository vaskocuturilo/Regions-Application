//
//  KazakhstanViewController.swift
//  Regions-Application
//
...
class KazakhstanViewController: UIViewController, UIGestureRecognizerDelegate, UITextFieldDelegate {
    ...
    override func viewDidLoad() {
        ...
        textField.leftView = UIImageView(image: UIImage(named: "Kazakhstan_Flag"))
    ...
    private func personData() {
        apiService.responseRegion(endpoints: Constants.Endpoints.Kazakhstan,region: textField.text!) { [self](isSucess, str) in
            if isSucess {
                messages.showMessage(label: label, message: str)
                textField.text?.removeAll()
            } else if (str.contains("Could not connect to the server.")) {
                jsonLoad.loadSpeciesInfoJSOn(resource: "kazakhstanPerson", label: label, text: textField.text!)
                textField.text?.removeAll()
            } else {
                messages.showMessage(label: label, message: str)
                textField.text?.removeAll()
