...
class KosovoViewController: UIViewController, UIGestureRecognizerDelegate, UITextFieldDelegate {
    ...
    override func viewDidLoad() {
        ...
        textField.leftView = UIImageView(image: UIImage(named: "Kosovo-Flag"))
        ...
        showPopUp.isWasAlreadyShown(
            ...
            key: "isWasAlreadyShownKosovoPerson")
    }
    ...
    private func personData() {
        apiService.responseRegion(endpoints: Constants.Endpoints.Kosovo,region: textField.text!) { [self](isSucess, str) in
            if isSucess {
                messages.showMessage(label: label, message: str)
                textField.text?.removeAll()
            } else if (str.contains("Could not connect to the server.")) {
                jsonLoad.loadSpeciesInfoJSOn(resource: "kosovoPerson", label: label, text: textField.text!)
                textField.text?.removeAll()
            } else {
                messages.showMessage(label: label, message: str)
                textField.text?.removeAll()
