...
class FranceViewController: UIViewController, UIGestureRecognizerDelegate, UITextFieldDelegate {
    ...
    override func viewDidLoad() {
        ...
        textField.leftView = UIImageView(image: UIImage(named: "France-Flag"))
        ...
        showPopUp.isWasAlreadyShown(
            ...
            key: "isWasAlreadyShownFrancePerson")
    }
    ...
    private func personData() {
        apiService.responseRegion(endpoints: Constants.Endpoints.France,region: textField.text!) { [self](isSucess, str) in
            if isSucess {
                messages.showMessage(label: label, message: str)
                textField.text?.removeAll()
            } else if (str.contains("Could not connect to the server.")) {
                jsonLoad.loadSpeciesInfoJSOn(resource: "francePerson", label: label, text: textField.text!)
                textField.text?.removeAll()
            } else {
                messages.showMessage(label: label, message: str)
                textField.text?.removeAll()
