//
//  UzbekistanViewController.swift
//  Regions-Application
//
...
class UzbekistanViewController: UIViewController, UIGestureRecognizerDelegate, UITextFieldDelegate {
    ...
    override func viewDidLoad() {
        ...
        textField.rightView = UIImageView(image: UIImage(named: "Uzbekistan_Flag"))
        ...
        showPopUp.isWasAlreadyShown(
            ...
            key: "isWasAlreadyShownUzbekistanPerson")
    ...
    private func personData() {
        apiService.responseRegion(endpoints: Constants.Endpoints.Uzbekistan,region: textField.text!) { [self](isSucess, str) in
            if isSucess {
                messages.showMessage(label: label, message: str)
                textField.text?.removeAll()
            } else if (str.contains("Could not connect to the server.")) {
                jsonLoad.loadSpeciesInfoJSOn(resource: "uzbekistanPerson", label: label, text: textField.text!)
                textField.text?.removeAll()
            } else {
                messages.showMessage(label: label, message: str)
                textField.text?.removeAll()
